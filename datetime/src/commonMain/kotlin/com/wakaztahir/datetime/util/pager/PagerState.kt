...
internal class PagerState(
     ...
     */
    internal val interactionSource: InteractionSource
     ...
     */
    @get:IntRange(from = 0)
    internal val pageCount: Int by derivedStateOf {
    ...
    internal var currentPage: Int
        ...
        internal set(value) {
            if (value != _currentPage) {
                _currentPage = value
                if (DebugLog) {
                    println(message = "Current page changed: $_currentPage")
                }
            }
        }
     ...
     * To update the scroll position, use [scrollToPage] or [animateScrollToPage].
     */
    internal val currentPageOffset: Float by derivedStateOf {
    ...
    internal suspend fun animateScrollToPage(
     ...
     * Must be in the range -1f..1f.
     */
    internal suspend fun scrollToPage(
        @IntRange(from = 0) page: Int,
        @FloatRange(from = -1.0, to = 1.0) pageOffset: Float = 0f,
    ) {
        requireCurrentPage(page, "page")
    ...
    internal companion object {
         ...
         */
        internal val Saver: Saver<PagerState, *> = listSaver(
            save = {
                listOf<Any>(
                    it.currentPage,
                )
